...
inline void upd(pn rt){if(rt){rt->sz = g_s(rt->l) + g_s(rt->r) + 1; rt->v = min(rt->x,min(g_v(rt->l), g_v(rt->r)));}}
...
{if(!rt) return; pnt(rt->l, i); cout << rt->x + 1 /*<< ':' << i + g_s(rt->l) */<< ' '; pnt(rt->r, i + g_s(rt->l) + 1);}
int g_i(pn rt, int i)
{return !rt?0:(g_s(rt->l)>i?g_i(rt->l,i):(g_s(rt->l)==i?rt->x:g_i(rt->r,i-g_s(rt->l)-1)));}
void help(pn& rt, int l, int r){
    spl(rt, x1, x3, r + 1); spl(x1, x1, x2, l);
    mer(rt, x2, x1); mer(rt, rt, x3);
}
int main(){
    int n1, m, a, b;
    cin >> n1 >> m;
